...
/**
 * Represents the third output port of a function unit at flow design implementations in Scala.
 ...
 *
 */
trait OutputPort3[T] extends FunctionUnit {
  
  /**
   * The human readable name of this trait output port.
   * May be overridden by the function unit mixing in this trait. Default is "output3"
   */
  private[this] var portName = "output3"
   ...
   */
  private[this] def output3IsProcessedBy(operation: T => Unit) {
   ...
   */
  private[this] def forwardOutput3(msg: T) {
   ...
   * <i>fu.output3</i> -> <i>receiver</i>.
   */  
  val output3 = new de.grammarcraft.scala.flow.dsl.OutputPort[T](output3IsProcessedBy(_), forwardOutput3(_))

  /**
   * Flow DSL element to define user named output port which may be used instead 
   * of [[de.grammarcraft.scala.flow.OutputPort3[T].output3]] when connecting function unit ports.<br>
   * Typically the definition is done as follows:<br>
   * <code>val <i>myPortName</i> = OutputPort3("<i>myPortName</i>")</code>.
   ...
   */  
  def OutputPort3(userPortName: String): dsl.OutputPort[T] = {
