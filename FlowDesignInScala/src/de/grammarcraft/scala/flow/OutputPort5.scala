...
/**
 * Represents the fifth output port of a function unit at flow design implementations in Scala.
 ...
 *
 */
trait OutputPort5[T] extends FunctionUnit {
  
  /**
   * The human readable name of this trait output port.
   * May be overridden by the function unit mixing in this trait. Default is "output5"
   */
  private[this] var portName = "output5"
   ...
   */
  private[this] def output5IsProcessedBy(operation: T => Unit) {
   ...
   */
  private[this] def forwardOutput5(msg: T) {
   ...
   * <i>fu.output5</i> -> <i>receiver</i>.
   */  
  val output5 = new de.grammarcraft.scala.flow.dsl.OutputPort[T](output5IsProcessedBy(_), forwardOutput5(_))

  /**
   * Flow DSL element to define user named output port which may be used instead 
   * of [[de.grammarcraft.scala.flow.OutputPort5[T].output5]] when connecting function unit ports.<br>
   * Typically the definition is done as follows:<br>
   * <code>val <i>myPortName</i> = OutputPort5("<i>myPortName</i>")</code>.
   ...
   */  
  def OutputPort5(userPortName: String): dsl.OutputPort[T] = {
